...
class GaussianUpsampling(nn.Module):
    ...
    def forward(self, x, durations, range_outputs, max_len):
        range_outputs = range_outputs.unsqueeze(-1)
        ...
        t = torch.arange(0, torch.max(
            s)).unsqueeze(0).unsqueeze(1).to(device)  # [1, 1, T]
        ...
        output = torch.matmul(w.transpose(
            1, 2), x)  # [B, T, ENC_DIM]
